...
fixtures: ACONST_NULL, CHECKCAST, INSTANCEOF, F/DLOAD_n, F/DSTORE_n,
narrowing conversions, shifts, FADD/DADD/FSUB/DSUB/FMUL/DMUL,
DUP_X1. Adding all these in `src/Aot/Ir/Builder.php` (no new IR node
types — reused existing `NullLit`/`FloatLit`/`StringLit`/`BinOp`/
`StaticCall`) lifted coverage to 86.5%.

**Architectural verdict:** the IR Builder's design holds against
production bytecode. Gaps are well-known JVM opcodes (mechanical
fills) plus the already-roadmapped sub-step 1c-β. Doing 1c-β would
lift IR coverage to ~99% on commons-lang3.
