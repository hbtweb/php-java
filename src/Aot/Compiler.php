...
final class Compiler
{
    ...
    private function compileFromGenericClass(JavaCompiledClass $jcc, string $classPath): string
    {
        ...
        foreach ($methods as $method) {
            ...
            // Prefer IR-path; fall back to string-path on opcodes the
            // IR Builder doesn't yet handle. The IR-path is rank-1
            // verified faster (2× on BenchInvoke) and produces cleaner
            // emit; opcode coverage is at 9/9 fixtures but unknown in
            // the wild, so the fallback is safety.
            $emitted = $this->tryCompileViaIr(
                $jcc, $classPath, $name, $desc,
                $codeAttr->getCode(), $codeAttr->getExceptionTables()
            );
            if ($emitted === null) {
                $emitted = $this->compileMethod(
                    $classPath, $name, $desc,
                    $codeAttr->getCode(), $codeAttr->getExceptionTables()
                );
            }
            $emittedMethods[] = $emitted;
        }

        // Merge IR-Builder-generated lambda classes into the global
        // lambdaClasses list so the module-level append picks them up
        // alongside string-path-generated lambdas.
        if ($this->irBuilder !== null) {
            foreach ($this->irBuilder->getLambdaClasses() as $lc) {
    ...
    /**
     * Try the IR path. Returns the formatted method string on success,
     * null if the IR Builder hits an unsupported opcode (caller falls
     * back to string-path). Lambda classes generated by the IR Builder
     * are merged into $this->lambdaClasses for the module-level emit.
     */
    private function tryCompileViaIr(
        JavaCompiledClass $jcc, string $classPath, string $methodName,
        string $descriptor, string $bytecode, array $exceptionTables
    ): ?string {
        try {
            // Lazy-init: one Builder per compileFromGenericClass call,
            // shared across methods so the BootstrapMethods cache and
            // lambda counter stay coherent.
            ...
            $method = $this->irBuilder->buildMethod(
                $jcc, $methodName, $descriptor, $bytecode,
                str_replace('.', '/', $classPath), $exceptionTables
            );
            // Register inlinable methods so the cross-method inline
            // pass at the end of compileFromGenericClass can substitute
            // self::<name>(args) call sites. Detection: single BB, no
            // stmts, Return_ terminator with a pure expression.
            $this->detectInlinableFromIr($method);

            return $this->irLowerer->lowerMethod($method);
        } catch (\Throwable $e) {
            // IR Builder hit an opcode it doesn't handle, or a build-
            // time issue. Caller will retry via the string path.
